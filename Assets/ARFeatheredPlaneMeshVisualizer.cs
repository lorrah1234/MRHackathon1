...
public class ARFeatheredPlaneMeshVisualizer : MonoBehaviour
{
    ...
    [SerializeField]
    float m_FeatheredWidth = 0.2f;
    public float featheringWidth
    {
        get { return m_FeatheredWidth; } 
        set {  m_FeatheredWidth = value; }
    }
    ...
    void GenerateBoundaryUVs(Mesh mesh)
    {
        ...
        Vector3 centreInPlaneSpace = s_Vertices[s_Vertices.Count - 1];
        Vector3 uv = new Vector3(0, 0, 0);
        float shortestUVMapping = float.MaxValue;
        for (int i = 0; i < vertexCount - 1; i++)
        {
            float vertexDist = Vector3.Distance(s_Vertices[i], centreInPlaneSpace);
            float uvMapping = vertexDist / Mathf.Max(vertexCount - featheringWidth, 0.001f);
            uv.x = uvMapping;
